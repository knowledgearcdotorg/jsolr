...
{	
	...
	public function loadResultsTemplate()
	{
    	...
    	if ($extension) {
	    	$pluginOverridePath = JPATH_PLUGINS.DS."jsolrsearch".DS.$extension.DS.'views';
	    	$themeOverridePath = JPATH_ROOT.DS.'templates'.DS.
	    			JFactory::getApplication()->getTemplate().DS.'html'.DS.
	    			'com_jsolrsearch'.DS.'plugins'.DS.$extension;
	        ...
	        if (array_search($themeOverridePath.DS.'views', $templates) == false && 
	    		is_dir($themeOverridePath.DS.'views')) {
		    	$this->addTemplatePath($themeOverridePath);
		    }
    	}
